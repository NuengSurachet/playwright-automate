...
test.describe("Setup Company", () => {
      ...
      test("Add new company", async () => {
        ...
        await page.getByRole("treeitem", { name: "คลองเตย" }).click({ force: true });
        ...
        await page.locator('input[name="email"]').fill("test@gmail.com", { force: true });
     ...
     await page.pause();
       ...
       await page.getByRole('link', { name: 'Setup Company' }).click({ force: true });
       await page.getByPlaceholder('Type to filter...').click({ force: true });
       await page.getByPlaceholder('Type to filter...').fill('MD105',{ force: true });
       await page.getByPlaceholder('Type to filter...').click({ force: true });
